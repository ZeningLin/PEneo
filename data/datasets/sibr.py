...
class SIBRDataset(Dataset):
    """Dataset class for SIBR"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.annotation_root = os.path.join(data_root, "converted_label")

        dir_split_file = os.path.join(data_root, f"{split}.txt")
        with open(dir_split_file, "r") as f:
            annotation_fname_list = f.readlines()
            annotation_fname_list = [
                os.path.basename(x.strip()) for x in annotation_fname_list
            ]
        self.annotation_fname_list = annotation_fname_list
    ...
    def __len__(self) -> int:
        return len(self.annotation_fname_list)

    def __getitem__(self, index):
        annotation_fname = self.annotation_fname_list[index]
        document_info = json.load(
            open(
                os.path.join(self.annotation_root, annotation_fname),
                "r",
                encoding="utf-8",
            )
        )
