...
class LiltLayoutEmbeddings(nn.Module):
    def __init__(self, config):
        super(LiltLayoutEmbeddings, self).__init__()
...
class LiltLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = LiltAttention(config)
...
@dataclass
class LiltModelOutput(BaseModelOutputWithPoolingAndCrossAttentions):
    semantic_output: torch.FloatTensor = None
    layout_output: torch.FloatTensor = None
...
class LiltModel(LiltPreTrainedModel):
    ...
    def forward(
        ...
        last_hidden_states = torch.cat([sequence_output, layout_encoder_outputs], -1)
        return LiltModelOutput(
            ...
            layout_output=layout_encoder_outputs,
        )
